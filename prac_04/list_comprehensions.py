...
for name in full_names:
    lower_name = full_names[count].lower()
    count = count + 1
...
ten_plus = [number for number in numbers if number > 9]
print(ten_plus)
...
long_names = [last_name for last_name in full_names if len(last_name) > 11]
for e in long_names:
    split_names = long_names[e].split()
    print(split_names)
# last_names = [long_names[1].split() for i in long_names]
